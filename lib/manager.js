...
export async function buildArchives(startId = LEAGUE_ID) {
  ...
  for (const s of chain) {
    ...
    // "Best Regular Season" is an ALL-TIME record — only count a season once its
    // regular season is COMPLETE, so an in-progress current season (e.g. a 3-0
    // start = 1.000 win%) can't temporarily hijack the record.
    const bestRegEligible = regularSeasonDone(
      league,
      nflState,
