...
export async function buildManagerProfile(userId, startId) {
  ...
  for (const s of chain) {
    ...
    const [weekData, txData, draft, placements] = await Promise.all([
...
export async function buildOwners(startId = LEAGUE_ID) {
  ...
  // Former managers to hide from the Owners directory.
  const HIDE_OWNERS = new Set([
    "993557703488221184", // nocon7
    "994770614504218624", // jojodacircusboy
  ]);
  const HIDE_NAMES = new Set(["nocon7", "jojodacircusboy"]);

  return Object.values(m)
    .filter((r) => !HIDE_OWNERS.has(String(r.userId)) && !HIDE_NAMES.has(r.name))
    .map((r) => ({
      ...r,
...
export async function buildArchives(startId = LEAGUE_ID) {
  ...
  for (const s of chain) {
    ...
    const worstRoster = [...rosters].sort((a, b) => {
      const aw = a.settings?.wins || 0, bw = b.settings?.wins || 0;
      if (aw !== bw) return aw - bw;
      return pts(a.settings?.fpts, a.settings?.fpts_decimal) - pts(b.settings?.fpts, b.settings?.fpts_decimal);
    })[0];
    const lastRid = worstRoster?.roster_id;
    champions.push({
      year,
