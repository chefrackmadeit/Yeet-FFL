...
// ---------------------------------------------------------------------------
// Weekly player projections (used to project each team's score for the Weekly
// Preview odds). This lives on api.sleeper.app but OUTSIDE the /v1 path, so it
// uses its own fetch rather than the get() helper above. Returns an array of
// per-player rows: { player_id, stats: { pts_ppr, pts_half_ppr, pts_std }, ... }.
// ---------------------------------------------------------------------------
...
// Build points + position lookups from a projections array. The scoring field
// is chosen from the league's reception setting: 1 = PPR, 0.5 = half, 0 = std.
export function projectionMaps(projections, recSetting = 0) {
  const field =
    recSetting >= 1 ? "pts_ppr" : recSetting > 0 ? "pts_half_ppr" : "pts_std";
  const pts = {};
  const pos = {};
  for (const e of projections || []) {
    const pid = e.player_id;
    if (e.stats && e.stats[field] != null) pts[pid] = e.stats[field];
    const p = e.player && e.player.position;
    if (p) pos[pid] = p;
  }
  return { pts, pos };
}

// Which real player positions can fill each flex-style slot.
const FLEX_ELIGIBLE = {
  FLEX: ["RB", "WR", "TE"],
  WRRB_FLEX: ["RB", "WR"],
  REC_FLEX: ["WR", "TE"],
...
// Best possible projected score from a set of player ids, respecting the
// league's starting slots (FLEX-aware). Used as a fallback when a manager
// hasn't set a lineup yet, so the odds never break on an empty lineup.
export function optimalProjection(playerIds, ptsMap, posMap, rosterPositions) {
  const slots = (rosterPositions || []).filter(
    (s) => s !== "BN" && s !== "IR" && s !== "TAXI"
  );
  const pool = (playerIds || [])
    .filter((id) => ptsMap[id] != null)
    .map((id) => ({ id, pts: ptsMap[id], pos: posMap[id] || "" }));
  const used = new Set();
  let total = 0;
  const takeBest = (eligible) => {
    ...
    for (const c of pool) {
      ...
      if (eligible.includes(c.pos) && (!best || c.pts > best.pts)) best = c;
    }
    if (best) {
      used.add(best.id);
      total += best.pts;
  ...
  slots.filter((s) => !FLEX_ELIGIBLE[s]).forEach((s) => takeBest([s]));
  slots.filter((s) => FLEX_ELIGIBLE[s]).forEach((s) => takeBest(FLEX_ELIGIBLE[s]));
  return Math.round(total * 10) / 10;
}

// A team's projected points for the week: sum its CURRENT set lineup, falling
// back to the best available lineup when the manager hasn't set one yet.
export function teamProjection(roster, ptsMap, posMap, rosterPositions) {
  const starters = (roster.starters || []).filter((p) => p && p !== "0");
  const needed = (rosterPositions || []).filter(
    (s) => s !== "BN" && s !== "IR" && s !== "TAXI"
  ).length;
  if (starters.length >= needed - 1) {
    let total = 0;
    for (const pid of starters) if (ptsMap[pid] != null) total += ptsMap[pid];
    return Math.round(total * 10) / 10;
  }
  return optimalProjection(roster.players, ptsMap, posMap, rosterPositions);
}
...
// ---------------------------------------------------------------------------
// True once a season's REGULAR season is finished, so a last-place ("burger")
// designation is meaningful. Past seasons are always done; the current season
// counts as done once the NFL reaches the league's playoff start week (or the
// league is already marked complete / we're in the postseason).
export function regularSeasonDone(league, nflState, isCurrent) {
  if (!isCurrent) return true;
  if (league?.status === "complete") return true;
  if (nflState?.season_type === "post") return true;
  const pw = Number(league?.settings?.playoff_week_start) || 0;
  const wk = Number(nflState?.week) || 0;
  return pw > 0 && wk >= pw;
}
...
export async function getSeasonStandingsSets(startId = LEAGUE_ID) {
  ...
  for (const s of chain) {
    const isCurrent = s.leagueId === chain[0].leagueId;
    const rows = await getStandings(s.leagueId);
    const regDone = regularSeasonDone(s.league, nflState, isCurrent);
    sets.push({
      season: s.season,
      leagueId: s.leagueId,
      isCurrent,
      champRosterId: s.championRosterId,
      // Only crown a "burger" (last place) once the regular season is over.
      loserRosterId: regDone && rows.length ? rows[rows.length - 1].rosterId : null,
      rows,
    });
  }
  return sets;
...
export async function getReigningAwards(startId = LEAGUE_ID) {
  const chain = await getSeasonChain(startId);
  for (const s of chain) {
    // Only crown from a season that has actually FINISHED. The in-progress
    // current season can carry a stale winner id and would otherwise report
    // its current-standings last-place team as the "burger" — skip it.
    if (s.league?.status !== "complete") continue;
    if (!s.championRosterId) continue; // no winner recorded
    const standings = await getStandings(s.leagueId);
    const champ = standings.find(
      (r) => String(r.rosterId) === String(s.championRosterId)
    );
    // Burger = worst regular-season record (standings sort by wins, then PF).
